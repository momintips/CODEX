...
udp() {
    if check_network; then
        messages=$(curl -s "$MOMIN/check_version" | jq -r --arg vs "$version" '.[] | select(.message == $vs) | .message')
...
PUT 10 ${var4}
echo -e "\e[32m[\e[0m\uf489\e[32m] \e[36mMOMIN \e[36m1 1.4\e[0m"
PUT 12 0
ads1=$(curl -s "$MOMIN/ads" | jq -r '.[] | .message')
